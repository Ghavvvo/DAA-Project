...
\begin{document}
    ...
    \subsection{Esquemas de Aproximación en Tiempo Polinomial (PTAS)}
    ...
    \begin{enumerate}
    \item \textbf{Herencia de APX-hardness}: MWNP es conocido por ser \textbf{APX-hard} para un número variable de grupos $k$ \cite{korf1998optimal}. Dado que nuestro problema es \textbf{estrictamente más restrictivo} que MWNP (toda solución con $D_{max}$ pequeña automáticamente tiene suma máxima cercana al promedio, pero no viceversa), hereda esta propiedad de APX-hardness.
    ...
    \textbf{Eliminación de instancias infactibles:} Si existe algún artículo cuyo peso supera la capacidad máxima entre todas las mulas (w_i > max_j C_j), la instancia se declara inmediatamente infactible, ya que dicho artículo no puede ser asignado a ninguna mula sin violar las restricciones de capacidad.

    \textbf{Fijación de artículos de asignación única:} Para cada artículo que solo puede ser cargado por una única mula (es decir, w_i ≤ C_j para exactamente una mula j y w_i > C_k para toda k ≠ j), se fija su asignación de antemano (x_ij = 1). Esto reduce el número de variables de decisión y simplifica la instancia restante.

    Estas reglas se aplican en tiempo O(n·m) y permiten descartar casos triviales, además de reducir el espacio de búsqueda en problemas con restricciones de capacidad ajustadas. Si bien esta kernelización básica no garantiza un tamaño polinomial del kernel, en la práctica logra una reducción significativa para instancias con artículos "grandes" o capacidades heterogéneas.
    ...
    \begin{algorithm}[H]
        ...
        \begin{algorithmic}[1]
            ...
            \State \textbf{// --- FASE 1: CONSTRUCCIÓN VORAZ (GREEDY) ---}
            \State Ordenar $A$ descendentemente por valor
            \State
            \For{cada articulo $art$ en $A$}
                \State $Mula\_Candidata \gets$ NULO
                ...
                \EndFor
                \State
                \If{$Mula\_Candidata$ NO ES NULO}
                    \State Asignar $art$ a $Mula\_Candidata$
                \Else
                    \State \Return ERROR ``Imposible asignar''
                \EndIf
            \EndFor
            \State
            \State \textbf{// --- FASE 2: REFINAMIENTO (BÚSQUEDA LOCAL) ---}
            \State $Mejora\_Encontrada \gets$ VERDADERO
            \State
            \While{$Mejora\_Encontrada$ ES VERDADERO}
                \State $Mejora\_Encontrada \gets$ FALSO
                \State $Mula\_Max \gets$ Mula con mayor $valor\_total$
                \State $Mula\_Min \gets$ Mula con menor $valor\_total$
                \State $Diferencia\_Actual \gets Mula\_Max.valor - Mula\_Min.valor$
                \State
                \State \textbf{// Intento 1: Mover un artículo de Max a Min}
                \For{cada articulo $a$ en $Mula\_Max.articulos$}
                    \If{$(Mula\_Min.peso + a.peso \leq Mula\_Min.capacidad)$}
                        \State $Nueva\_Dif \gets$ CalcularDiferenciaSiMovemos$(a, Mula\_Max, Mula\_Min)$
                        \If{$Nueva\_Dif < Diferencia\_Actual$}
                            \State Mover $a$ de $Mula\_Max$ a $Mula\_Min$
                            \State $Mejora\_Encontrada \gets$ VERDADERO
                            \State \textbf{romper} \Comment{Reiniciar el bucle principal}
                        \EndIf
                    \EndIf
                \EndFor
                \State
                \State \textbf{// Intento 2: Swap (Si el movimiento simple falló)}
                \If{NO $Mejora\_Encontrada$}
                    \For{cada $a$ en $Mula\_Max$ Y cada $b$ en $Mula\_Min$}
                        \If{IntercambioEsValido$(a, b, Mula\_Max, Mula\_Min)$}
                            \State $Nueva\_Dif \gets$ CalcDifSiIntercambiamos$(a, b)$
                            \If{$Nueva\_Dif < Diferencia\_Actual$}
                                \State Intercambiar$(a, b)$
                                \State $Mejora\_Encontrada \gets$ VERDADERO
                                \State \textbf{romper}
                            \EndIf
                        \EndIf
