...
\begin{document}
    ...
    Dado un conjunto de $N$ artículos $A = \{a_1, \ldots, a_n\}$ y $M$ mulas $M = \{m_1, \ldots, m_m\}$, buscamos una partición tal que:
    ...
    A continuación, detallamos el primer eslabón de esta cadena para justificar el uso de PARTITION como problema intermedio en nuestra demostración.

    \paragraph{Reducción de SUBSET-SUM a PARTITION}
    El problema de la Suma de Subconjuntos (SUBSET-SUM) se define así: dado un conjunto finito de enteros $A = \{a_1, \ldots, a_n\}$ y un entero objetivo $t$, ¿existe un subconjunto $A' \subseteq A$ tal que la suma de sus elementos sea exactamente $t$?
    ...
    \subsubsection{Reducción de PARTITION a Transporte Discreto}
    Una vez establecido que PARTITION es NP-duro, procedemos a reducirlo a nuestro problema de interés. El problema PARTITION se define de la siguiente manera: dado un multiconjunto $S$ de enteros positivos, ¿se puede particionar $S$ en dos subconjuntos $S_1$ y $S_2$ tal que la suma de los elementos en $S_1$ sea igual a la suma de los elementos en $S_2$?
