...
\begin{document}
    ...
    \begin{enumerate}
        ...
        \]
        Cada mula debe llevar al menos un artículo (condición explícita del enunciado: ``utilizando el número exacto de mulas requerido'').
    \end{enumerate}
    ...
    \begin{algorithm}[H]
        ...
        \begin{algorithmic}[1]
            ...
            \While{$Mejora$ ES VERDADERO}
                ...
                \State $M_{\text{max}} \gets$ Mula con mayor valor total
                \State $M_{\text{min}} \gets$ Mula con menor valor total
                \State $Dif_{\text{actual}} \gets M_{\text{max}}\text{.valor} - M_{\text{min}}\text{.valor}$
                \State
                \State \textbf{// Intento 1: Mover artículo}
                \For{cada $a$ en $M_{\text{max}}\text{.articulos}$}
                    \If{$M_{\text{min}}$ puede cargar $a$ Y mejora la diferencia}
                        \State Mover $a$ de $M_{\text{max}}$ a $M_{\text{min}}$
                        \State $Mejora \gets$ VERDADERO
                        \State \textbf{break}
                    \EndIf
                \EndFor
                ...
                \If{NO $Mejora$}
                    \For{cada $a$ en $M_{\text{max}}$ Y cada $b$ en $M_{\text{min}}$}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.85\textwidth]{experimento1_calidad}
        \caption{Comparación de calidad: La Búsqueda Local alcanza el óptimo en más del 50\% de las instancias, superando significativamente a la heurística voraz.}
    \end{figure}
    ...
    Considerando un minuto como límite razonable para aplicaciones prácticas, el análisis determina que el algoritmo de fuerza bruta puede resolver instancias de hasta 17 artículos distribuidas entre 4 mulas, requiriendo aproximadamente 50 segundos para el caso más exigente dentro de este umbral. Este límite, correspondiente a un producto $N \times M$ de aproximadamente 50 unidades, marca la frontera donde la explosión combinatoria inherente al problema ---con su crecimiento exponencial $m^n$--- se vuelve computacionalmente inmanejable.

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.85\textwidth]{analisis_limite_fuerza_bruta}
        \caption{Límite de escalabilidad: La fuerza bruta alcanza su límite práctico en $N \times M \approx 50$, mientras que las heurísticas mantienen tiempos despreciables incluso para instancias grandes.}
    \end{figure}
